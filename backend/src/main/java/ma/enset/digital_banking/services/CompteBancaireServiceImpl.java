...
public class CompteBancaireServiceImpl implements CompteBancaireService {
    ...
    @Override
    public List<ClientDTO> rechercher(String motCle) {
        List<Client> clients=clientRepository.findByNameContains(motCle);
        List<ClientDTO> clientDTOS = clients.stream().map(clt->
